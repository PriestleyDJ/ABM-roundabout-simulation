...
namespace ABMTrafficSimulation {
    public static class TrafficSystemGizmos {
        ...
        private static void DrawGizmo(TrafficNetwork script, GizmoType gizmoType) {
            //Don't go further if we hide gizmos
            if (!script.showGizmos) {
                return;
            }

            foreach (LaneSegmentation segment in script.lanes) {
                //Draw segment names
                ...
                for (int j = 0; j < segment.waypoints.Count; j++) {
                    ...
                    Vector3 p = segment.waypoints[j].GetPos();
                    ...
                    if (j < segment.waypoints.Count - 1 && segment.waypoints[j + 1] != null) {
                        pNext = segment.waypoints[j + 1].GetPos();
                    }
                ...
                foreach (LaneSegmentation nextSegment in segment.nextLanes) {
                    if (nextSegment != null){
                        Vector3 p1 = segment.waypoints.Last().GetPos();
                        Vector3 p2 = nextSegment.waypoints.First().GetPos();
        ...
        private static int GetArrowCount(Vector3 pointA, Vector3 pointB, TrafficNetwork script) {
